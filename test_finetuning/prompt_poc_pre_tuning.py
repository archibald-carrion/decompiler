...
LOCAL_MODEL_DIR = os.path.join(SCRIPT_DIR, "models", "distilgpt2")
...
def load_model():
    ...
    print(f"Loading DistilGPT2 model to local directory: {LOCAL_MODEL_DIR}")
    ...
    if os.path.exists(os.path.join(LOCAL_MODEL_DIR, "config.json")):
        ...
        print("⬇️  Downloading model for first time...")
        # Download and save to local directory
        tokenizer = GPT2Tokenizer.from_pretrained('distilgpt2', cache_dir=None)
        model = GPT2LMHeadModel.from_pretrained('distilgpt2', cache_dir=None)
...
def cleanup_local_files():
    ...
    if os.path.exists(LOCAL_MODEL_DIR):
        ...
        print(f"✓ Removed local model directory: {LOCAL_MODEL_DIR}")
    else:
        print("No local model files to remove")
